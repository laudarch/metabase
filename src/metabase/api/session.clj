...
(api/defendpoint POST "/stakeholder/login"
  "Login into stakeholders",
  [:as {{:keys [username password] :as body} :body}]
  {username       su/NonBlankString
   password        su/NonBlankString}
  (def b (format "{\"Username\":\"%s\",\"Password\":\"%s\",\"UserType\":\"internal\"}" username password))
  (def res (client/post "https://stakeholder.icums.gov.gh/api/unipass/userAuthentication"
                        {
                          :body b
                          :accept :json,
                          :content-type :json
                          :headers {:Auth-Id "56BB6DAF9362F", :Auth-Secret "w2AQbhwvNdUzeh0MjWa8tOyfRx5IpUrkrkpuBCBzd7a0tf8pT0uPLf049zRUgUGJ",:accept :json, :content-type :json}
                          }
                        ))
  {:response ( res :body)}
  )
